...
for article in articles: 
	...
	print() ### Ici, cette commande n'imprime rien...

	article = article.split(",") 

	media = (article[0:610][0]) ### Ici, il n'est pas nécessaire d'ajouter [0:610] après «articles»
	### Les parenthèses ne sont pas nécessaires non plus
	### Donc tu aurais pu écrire plus simplement:
	media = article[0] ### Cette variable «media» remplace celle qui a été déclarée trois lignes plus haut

	# print(media)

	section = (article[0:610][1])
	section = article[1].strip() ### J'ajoute «strip()» pour retrancher les espaces blancs superflus
	# print(section)

	mot = (article[0:610][2])
	mot = article[2].strip()

	mots = mot.replace("samedi 26 janvier 2019", "")
	# print(mots)

	pages = (article[0:610][2])
	pages = article[2]
	#print(pages[1:3])
	#print(pages[-3:])
	#pages2 = pages.replace("_",)
	#print(pages2)
	#print(pages2[1:3])
	#print(pages2[-3:])

	y = (article[0:610][-1])
	page = y.replace("p. ", "") ### Astucieuse façon de retrancher quelque chose qui revient fréquemment (le «p.») et qui n'est peut-être pas si utile
	...
	# print(media + " a publié, dans la section" + section + " à la page " + page  + ", un article de" + mots  +".")
...
for nombre in articles:
	nombre = nombre.split(",")
	aA = nombre[0:610][0].count("Le Devoir")
	if aA == 1:
		a += 1


	bB = nombre[0:610][0].count("La Presse+")
	if bB == 1:
		b += 1

	cC = nombre[0:610][0].count("Le Soleil")
	if cC == 1:
		c += 1	

	dD = nombre[0:610][0].count("Le Nouvelliste")
	if dD == 1:
		d += 1	

	eE = nombre[0:610][0].count("La Tribune")
	if eE == 1:
		e += 1	

	fF = nombre[0:610][0].count("La Voix de l'Est")
	if fF == 1:
		f += 1	
...
# print("Le Devoir a publié " + a + " articles le samedi 26 janvier 2019")
# print("La Presse+ a publié " + b + " articles le samedi 26 janvier 2019")
# print("Le Soleil a publié " + c + " articles le samedi 26 janvier 2019")
# print("Le Nouvelliste a publié " + d + " articles le samedi 26 janvier 2019")
# print("La Tribune a publié " + e + " articles le samedi 26 janvier 2019")
# print("La Voix de l’Est a publié " + f + " articles le samedi 26 janvier 2019")
